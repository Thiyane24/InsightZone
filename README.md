...
[User Interface: WhatsApp] ---> [File/Text Payload] ---> [Meta Cloud API Webhook]
                                                                  |
                                                  (Rate Limiter / HMAC SHA 256 Verification)
                                                                  v
...
**Ingestion and Perimeter Layer:** The client submits a data payload (CSV, Excel, machine readable PDF, or raw text). The FastAPI webhook intercepts the HTTPS POST request, applies rate limiting to protect against abuse and excessive traffic, executes cryptographic signature verification, and delegates the payload to an asynchronous background worker thread.

**Bronze Layer (Raw Ingestion):** The raw file or text input is ingested into memory. A dictionary-based heuristic routing mechanism detects and maps columns based on synonymous terminology across multiple languages (e.g., Date, Data, Item, Price, Preço, Qty, Total).

**Silver Layer (Validation and Cleaning):** The data is structured into Pandas DataFrames. Type enforcement is applied rigidly, and anomalous rows such as missing primary fields, corrupted records, or negative transactional quantities are systematically isolated and purged.

**Gold Layer (Analytics and Persistence):** Statistical aggregations and Key Performance Indicators (KPIs) are computed. Charts and visual summaries are generated using Matplotlib for inclusion in the final report. Business metrics and client interaction states are persisted in a relational PostgreSQL instance. Dataframe memory allocation is instantly optimized via explicit garbage collection (`gc.collect()`) execution.

**Artifact Generation and Dispatch:** The final analytical report is compiled into a PDF binary stream utilizing ReportLab lazy-loading modules, embedding the Matplotlib-generated charts, pushed to secure cloud storage via Cloudinary, and the permanent URL is dispatched back to the end-user via the Meta Cloud API.
...
To protect the service against abuse, excessive traffic, and potential denial-of-service conditions, the webhook layer enforces a rate limiter ahead of cryptographic validation and pipeline processing. This ensures that computing resources are reserved for legitimate, well-behaved traffic and that a single client cannot degrade service for others.

## Engineering Performance Guarantees and Telemetry

- **Perimeter Response Latency:** The ingestion webhook acknowledges payloads and returns an HTTP 200 OK status to the Meta API gateway in `<15ms`, mitigating network timeouts (HTTP 504) by offloading processing to non-blocking background workers.
- **Memory Optimization:** Implementing efficient DataFrame handling and deterministic garbage collection controls limits the container memory footprint to `<250MB`, ensuring operational stability and preventing Out Of Memory (OOM) faults during concurrent processing cycles.
...
- **Cryptographic Security:** The `/webhook` endpoint enforces perimeter validation by recalculating the HMAC SHA 256 signature using the application secret key against the incoming payload, rejecting unauthorized requests with an HTTP 403 status.
- **Rate Limiting:** Inbound requests are throttled at the webhook layer to prevent abuse and protect downstream processing resources from being overwhelmed by excessive or malicious traffic.
...
├── pipeline/
│   ├── __init__.py
│   ├── reader.py           # Heuristic normalization engine and text stream parsers (Pandas)
│   ├── metrics.py          # Statistical computation, Matplotlib chart generation, and Garbage Collection management
...
**Inbound Abuse Protection:** Beyond outbound retry protection, the webhook itself enforces a rate limiter at the perimeter to prevent any single source from overwhelming the system with excessive requests, ensuring fair and stable access for all legitimate users.
